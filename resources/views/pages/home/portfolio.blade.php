<?php
    $pboxs = [
        [
            'id'          => 'egg_pos_on_mobile',
            'title'       => 'EGG POS On Mobile',
            'url'         => 'http://eggdigital.com/egg-pos',
            'description' => 'eggdigital.com/egg-pos/',
        ],
        [
            'id'          => 'egg_pos_app',
            'title'       => 'EGG POS Application',
            'url'         => 'http://eggdigital.com/egg-pos',
            'description' => 'eggdigital.com/egg-pos/',
        ],
        [
            'id'          => 'online_ordering',
            'title'       => 'Online Ordering system',
            'url'         => 'https://online.eggsmartpos.com',
            'description' => 'online.eggsmartpos.com',
        ],
        [
            'id'          => 'virtual_pos_online',
            'title'       => 'Virtual POS Online system',
            'url'         => 'https://web.eggsmartpos.com',
            'description' => 'web.eggsmartpos.com',
        ],
        [
            'id'          => 'egg_pos_online',
            'title'       => 'EGG POS Online system',
            'url'         => 'http://eggdigital.com/egg-pos',
            'description' => 'eggdigital.com/egg-pos/',
        ],
        [
            'id'          => 'horeca_admin',
            'title'       => 'Merchant solutions Admin system',
            'url'         => 'http://cms.eggsmartpos.com',
            'description' => 'cms.eggsmartpos.com',
        ],
        [
            'id'          => 'portal2_oneplanets',
            'title'       => 'eProcurement solutions Portal system',
            'url'         => 'http://portal.oneplanets.com',
            'description' => 'portal.oneplanets.com',
        ],
        [
            'id'          => 'admin_oneplanets',
            'title'       => 'eProcurement solutions Admin system',
            'url'         => 'http://admin.oneplanets.com',
            'description' => 'admin.oneplanets.com',
        ],
        [
            'id'          => 'portal_wemall',
            'title'       => 'Portal website of wemall',
            'url'         => 'http://www.wemall.com',
            'description' => 'www.wemall.com',
        ],
        [
            'id'          => 'portal_itruemart',
            'title'       => 'Portal website of itruemart',
            'url'         => 'http://www.itruemart.com',
            'description' => 'www.itruemart.com',
        ],
        [
            'id'          => 'truelife',
            'title'       => 'Truelife Member system',
            'url'         => 'http://trueid.truelife.com/userv4/accounts/th',
            'description' => 'trueid.truelife.com/userv4/accounts/th',
        ],
        [
            'id'          => 'portal_wls',
            'title'       => 'Portal website of weloveshopping',
            'url'         => 'https://portal.weloveshopping.com',
            'description' => 'portal.weloveshopping.com',
        ],
        [
            'id'          => 'store_wls',
            'title'       => 'Seller Westore system',
            'url'         => 'https://store.weloveshopping.com',
            'description' => 'store.weloveshopping.com',
        ],
        [
            'id'          => 'wms_wls',
            'title'       => 'Westore Admin system',
            'url'         => 'https://wms.weloveshopping.com',
            'description' => 'wms.weloveshopping.com',
        ],
        [
            'id'          => 'youstore_wls',
            'title'       => 'Seller Youstore system',
            'url'         => 'http://youstore.weloveshopping.com',
            'description' => 'youstore.weloveshopping.com',
        ],
        [
            'id'          => 'ums_wls',
            'title'       => 'Youstore Dashboard',
            'url'         => 'http://ums.weloveshopping.com',
            'description' => 'ums.weloveshopping.com',
        ],
        [
            'id'          => 'sale_wls',
            'title'       => 'Youstore Sale system',
            'url'         => 'http://www.weloveshopping.com/sale',
            'description' => 'www.weloveshopping.com/sale',
        ],
        [
            'id'          => 'best_wls',
            'title'       => 'Yostore Admin system',
            'url'         => 'http://www.weloveshopping.com/best_admin_1',
            'description' => 'www.weloveshopping.com/best_admin_1',
        ],
        [
            'id'          => 'seller_wls',
            'title'       => 'Seller Westore front',
            'url'         => 'https://store.weloveshopping.com/siamit',
            'description' => 'store.weloveshopping.com/siamit',
        ],
        [
            'id'          => 'srusolution',
            'title'       => 'Saru Solution',
            'url'         => 'http://sarusolution.1mediasoft.net',
            'description' => 'sarusolution.1mediasoft.net',
        ],
        [
            'id'          => 'alpha_project',
            'title'       => 'Alpha Inter Placement',
            'url'         => 'http://alpha.igetproject.com',
            'description' => 'alpha.igetproject.com',
        ],
        [
            'id'          => 'staq01',
            'title'       => 'Digital Library 2ebook',
            'url'         => 'http://store.2ebook.com',
            'description' => 'store.2ebook.com',
        ],
        [
            'id'          => 'staq02',
            'title'       => 'CU Gateway',
            'url'         => 'http://staq.co.th/cugateway',
            'description' => 'staq.co.th/cugateway',
        ],
        [
            'id'          => 'staq03',
            'title'       => 'Maruey eLibrary',
            'url'         => 'http://maruey.2ebook.com',
            'description' => 'maruey.2ebook.com',
        ],
        [
            'id'          => 'staq04',
            'title'       => 'Textbook Publisher Management Information System',
            'url'         => 'http://pbms.2ebook.com',
            'description' => 'pbms.2ebook.com',
        ],
        [
            'id'          => 'staq05',
            'title'       => 'eDoc DRM Management',
            'url'         => 'http://drm.2ebook.com',
            'description' => 'drm.2ebook.com',
        ],
        [
            'id'          => 'staq06',
            'title'       => 'Publisher Management Software Design 1',
            'url'         => 'https://pbms.2ebook.com',
            'description' => 'pbms.2ebook.com',
        ],
        [
            'id'          => 'staq07',
            'title'       => 'Publisher Management Software Design 2',
            'url'         => 'https://pbms.2ebook.com',
            'description' => 'pbms.2ebook.com',
        ],
        [
            'id'          => '2ebook_reader',
            'title'       => '2ebook Reader Software',
            'url'         => 'http://store.2ebook.com/',
            'description' => 'Ebook reader application',
        ],
        [
            'id'          => '2ebook_writer',
            'title'       => '2ebook Writter Software',
            'url'         => 'http://store.2ebook.com/',
            'description' => 'Ebook writer application',
        ]
    ];
?>      

    @include('pages.home.portfolio_box')
